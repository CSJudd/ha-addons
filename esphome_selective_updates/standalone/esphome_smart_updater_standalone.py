...
# ============================================================================
# CONFIGURATION PATHS - STANDALONE MODE
# ============================================================================

# Standalone mode: Paths can be overridden via environment variables
...
# HAOS addon: /config/esphome/device.yaml
# Codex standalone: /config/device.yaml (no esphome subdirectory)
CONTAINER_CONFIG_PATH = os.environ.get("ESPHOME_CONTAINER_PATH", "/config/esphome")
...
def compile_in_esphome_container(
    ...
    # Use container path from environment (handles both HAOS and standalone)
    container_yaml_path = f"{CONTAINER_CONFIG_PATH}/{yaml_name}"

    rc, _ = docker_exec(
        container,
        ["esphome", "compile", container_yaml_path],
        capture=False
    )
...
def ota_upload_via_esphome(
    ...
    args = ["esphome", "upload", container_yaml_path, "--device", target]
